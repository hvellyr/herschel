...
template <>
struct NodeAnnotator<std::shared_ptr<ScopeNode>> {
  static void annotate(Annotator* ann, std::shared_ptr<ScopeNode> node)
  {
    ann->annotateNodeList(node->children(), !K(markTailPos), !K(markSingleType));
  }
};
    ...
    if (!var) {
      ...
      if (!funcdef->isGeneric()) {
        HR_LOG(kError, node->srcpos(), E_BadGenericReferrer)
            << "Bad method '" << node->name()
            << "' binding type (referred symbol is not a generic function).";
      ...
      HR_LOG(kError, node->srcpos(), E_BadGenericReferrer)
          << "Bad method '" << node->name()
          << "' binding type (referred symbol is not a generic function [2]).";
      HR_LOG(kError, var->srcpos(), E_BadGenericReferrer)
