...
class MacroPattern {
public:
  MacroPattern(const SrcPos& srcpos, const TokenVector& pattern, const TokenVector& replc)
      : fSrcPos(srcpos)
      , fPattern(pattern)
      , fReplacement(replc)
  {
  ...
  MacroPattern(const MacroPattern& other) { *this = other; }
...
enum MacroType { kMacro_Invalid, kMacro_Any, kMacro_Def, kMacro_Function };
...
public:
  ...
  void mixinPatternPart(std::shared_ptr<SyntaxTreeNode> patternTree,
                        const TokenVector& pattern, const TokenVector& rplcmtn);
  void mixinPattern(const String& macroName, const TokenVector& pattern,
...
public:
  ...
  std::shared_ptr<SyntaxTable> syntaxTable() const { return fSyntaxTable; }

  MacroType type() const { return fMacroType; }
