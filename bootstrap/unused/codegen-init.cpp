...
ModuleRuntimeInitializer::makeClassAllocCall(const Type& ty) const
{
  ...
  if (isa.isIntersection()) {
    argv.push_back(llvm::ConstantInt::get(context(),
                                          llvm::APInt(32, isa.intersectionTypes().size(), true)));
    // arg 5-n: the inherited classes/types
    for (size_t i = 0; i < isa.intersectionTypes().size(); i++)
      argv.push_back(makeGetTypeLookupCall(isa.intersectionTypes()[i]));
  }
  else if (isa.isDef()) {
    argv.push_back(llvm::ConstantInt::get(context(),
